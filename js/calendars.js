...
$.calendars.newDate(1379, 10, 12, 'persian') */
		newDate: function(year, month, day, calendar, language) {
			...
			return calendar.newDate(year, month, day, 0, 0, 0);
		},
	...
	/** Generic date, based on a particular calendar.
		...
		@throws Error if an invalid date. */
	function CDate(calendar, year, month, day, hour, minute, second) {
...
date.newDate(2001, 1, 1) */
		newDate: function(year, month, day, hour, minute, second) {
			return this._calendar.newDate((typeof year === 'undefined' || year === null ? this : year), month, day, hour, minute, second);
		},
...
var month = date.month() */
		month: function(month) {
			if (arguments.length === 0) { return this._month; 
			}else if (month === 0) {
				this.set(this._year - 1, 'y');
				this.set(12, 'M');
				return this.set(1, 'd');
			}else if (month === 13) {
				this.set(this._year + 1, 'y');
...
var day = date.day() */
		...
		hour: function(hour) {
			return (arguments.length === 0 ? this._hour : this.set(hour, 'H'));
		},

		minute: function(minute) {
			return (arguments.length === 0 ? this._minute : this.set(minute, 'm'));
		},

		second: function(second) {
			return (arguments.length === 0 ? this._second : this.set(second, 's'));
		},
	 ...
	 */
	 	formatDate: function(format) {
			...
			var m=this._minute;
			var s=this._second;
			if (typeof H === 'undefined' || typeof m === 'undefined' || typeof s === 'undefined'){
				var now_time = new Date();
				H = now_time.getHours();
				m = now_time.getMinutes();
				s = now_time.getSeconds();
			}
			...
			value["NNN"]=monthNamesShort[M-1];
			value["d"]=d;
			value["dd"]=this.LZ(d);
			value["E"]=dayNamesShort[E];
			value["EE"]=dayNames[E];
			value["H"]=H;
			value["HH"]=this.LZ(H);
			if (H==0){value["h"]=12;}
...
var today = calendar.newDate() */
		newDate: function(year, month, day, hour, minute, second) {
			...
			if (month === 0 ){
				return new CDate(this, year - 1, 12, day, hour, minute, second);
			}else if(month === 13){
				return new CDate(this, year + 1, 1, day, hour, minute, second);
			}else{
				return new CDate(this, year, month, day, hour, minute, second);
			}
		},
		...
		today: function() {
			var now = new Date();
			var nowDate = this.fromJSDate(now);
			nowDate.hour(now.getHours());
			nowDate.minute(now.getMinutes());
			nowDate.second(now.getSeconds());
			return nowDate;
		},
...
var info = calendar.extraInfo(2014, 1, 26) */
		...
		set: function(date, value, period) {
			...
			var h = (period === 'H' ? value : date.hour());
			var mm = (period === 'm' ? value : date.minute());
			var ss = (period === 's' ? value : date.second());
			if (period === 'y' || period === 'M') {
				d = Math.min(d, this.daysInMonth(y, m));
			}
			return date.date(y, m, d, h, mm, ss);
		},
